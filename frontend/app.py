...
Store_Size = st.selectbox("Store_Size", ["Small","Medium","High"]) #define the UI element for Store_Size
...
Store_Age_Years = st.number_input("Product Weight", min_value=0, value=300) #define the UI element for Store_Age_Years
...
if uploaded_file is not None:
    if st.button("Predict Batch", type="primary"):
        response = requests.post(f"{BACKEND_URL}/v1/salebatchpredict", files={"file": uploaded_file})  # Send file to Flask API
        if response.status_code == 200:
